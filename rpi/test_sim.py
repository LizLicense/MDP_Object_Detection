...
obstacle_android = [[5,4,180],[7,14,270],[12,9,90],[15,15,180],[15,4,270]]
...
class Simulator:
    ...
    def avoidobstacle(robot, self):
        ...
        time.sleep(1)
        #print('Go straight')
        Simulator.move_forward(self, robot)
        time.sleep(1)
        Simulator.move_forward(self, robot)
        time.sleep(1)
        #back12
    ...
    def move_forward(robot,self):
        self.handler.move(True, False)
        self.update_map()
        Simulator.move(self)
        if robot[2] == 0:        
            robot[1] = robot[1] + 1
            print('1x,y,z',robot[0],robot[1],robot[2])
        elif robot[2] == 90:
                #robot[2] = 180
            robot[0] = robot[0]+1
            print('2x,y,z',robot[0],robot[1],robot[2])
        elif robot[2] == 180:
                #robot[2] = 270
            robot[1] = robot[1] - 1
            print('3xy,z',robot[0],robot[1],robot[2])
        else:
            robot[0] = robot[0] - 1
            print('4x,y,z',robot[0],robot[1],robot[2])
        print('x,y,z',robot[0],robot[1],robot[2])
    ...
    def moveFinal(robot,endd,i,self):
        print('moveFinal======================>',i)
        end = endd[i]
        x1 = robot[0]
        x2 = end[0]
            ...
            if x2>x1: #right 
                ...
                if theta2 == 270:
                    ...
                    if theta1 == 0:
                        ...
                        Simulator.mov16(robot,end,self)
        time.sleep(3)
        #Simulator.avoidobstacle(robot,self)
        print('end',robot)
        proId = main1()
       #while proId >= 16:
        #    proId = main1()
        ...
        #print(displayList)
        #Simulator.avoidobstalce(robot,self)
        time.sleep(3)
        self.avoidobstacle(robot)
    ...
    def write_And(self,imageId,listObj,i):
        print('debug =================>',i,'\n')
        str_list = map(str, listObj[i])
        msg = f"{imageId},{','.join(str_list)},'automode'"
        print('msg',msg)
        write_android(msg) 
